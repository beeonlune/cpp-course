...
int main() {
    ...
    try {
        const std::size_t absurdly_huge_size = static_cast<std::size_t>(-1);
        void* mem = ::operator new(absurdly_huge_size);
        (void)mem;
    } catch (const std::exception& e) {
        std::cerr << e.what() << '\n';
    } catch (...) {
    ...
    try {
        ...
        double invalid_val = std::get<double>(var);
        (void)invalid_val;
    } catch (const std::exception& e) {
        std::cerr << e.what() << '\n';
    } catch (...) {
    ...
    try {
        ...
        int invalid_val = empty_opt.value();
        (void)invalid_val;
    } catch (const std::exception& e) {
        std::cerr << e.what() << '\n';
    } catch (...) {
    ...
    try {
        std::vector<int> vec;
        const std::size_t oversized = vec.max_size() + 1;
        vec.resize(oversized);
    } catch (const std::exception& e) {
        std::cerr << e.what() << '\n';
    } catch (...) {
        std::cerr << "Unknown exception\n";
